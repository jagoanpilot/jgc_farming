  ...
  return (
    <Switch>
      <Page>
        ...
        <StyledCardHeader>
            <Label text={`Welcome to JGPSafe, stake JGP to earn JGP.`}/>
        </StyledCardHeader>
        <div style={{fontWeight: 'bold', fontSize: 22, color: '#ffffff'}}>
            JGPSafe Currently has <span style={{color: '#4caf50', fontSize: 30}}>{parseFloat(getBalanceNumber(new BigNumber(totalSupply)).toFixed(2)).toLocaleString('en-US')}</span> JGP Staked
        </div>
        <Spacer size="md" />
        {!!account ? (
          <>
